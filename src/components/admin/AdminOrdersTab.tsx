...
import { Order, Partner, Assistant, City, Franchise, db } from '@/lib/supabase';
...
export const AdminOrdersTab: React.FC<AdminOrdersTabProps> = ({
  ...
  // Cities & Franchises state for snapshot resolution & badge display
  ...
  useEffect(() => {
    const loadMetadata = async () => {
      try {
        ...
        console.warn('Error loading cities/franchises for admin orders tab:', err);
      }
    };
    loadMetadata();
  }, []);
              ...
              <div key={order.id} className={`p-5 bg-white border rounded-2xl space-y-3 transition-all shadow-sm ${isLocked ? 'border-amber-400 bg-amber-50/20' : 'border-[#E5E7EB] hover:border-[#111111]/30'}`}>
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-3 border-b border-[#E5E7EB]">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <div className="font-bold text-[#111111] text-sm flex flex-wrap items-center gap-2">
                        ...
                        {(() => {
                          const cName = cities.find(c => c.id === order.city_id)?.name || (order as any).city || null;
                          const fName = franchises.find(f => f.id === order.franchise_id)?.name || null;
                          return (
                            <>
                              {cName && (
                                <span className="px-2 py-0.5 rounded-full text-[9px] font-bold bg-emerald-50 text-emerald-800 border border-emerald-200 flex items-center gap-1">
                                  <MapPin className="w-2.5 h-2.5" /> {cName}
                                </span>
                              )}
                              {fName && (
                                <span className="px-2 py-0.5 rounded-full text-[9px] font-bold bg-indigo-50 text-indigo-800 border border-indigo-200 flex items-center gap-1">
                                  <Building className="w-2.5 h-2.5" /> {fName}
                                </span>
                              )}
                            </>
                          );
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-white border border-[#E5E7EB] rounded-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto p-6 shadow-2xl space-y-5 relative text-xs">
            ...
            <div className="p-4 bg-[#F7F7F8] border border-[#E5E7EB] rounded-xl space-y-1.5">
              ...
              {(() => {
                const cName = cities.find(c => c.id === viewingOrder.city_id)?.name || (viewingOrder as any).city || null;
                const fName = franchises.find(f => f.id === viewingOrder.franchise_id)?.name || null;
                return (
                  <>
                    {cName && (
                      <div><span className="text-[#666666]">Şehir / Bölge:</span> <strong className="text-emerald-700">{cName}</strong></div>
                    )}
                    {fName && (
                      <div><span className="text-[#666666]">Yetkili Bayi (Snapshot):</span> <strong className="text-indigo-700">{fName}</strong></div>
                    )}
                  </>
                );
              })()}
