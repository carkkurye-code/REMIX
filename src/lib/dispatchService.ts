...
export class LiveDispatchService {
  ...
  public static async createOrderAndDispatch(input: CreateOrderDispatchInput): Promise<{ success: boolean; order?: Order; message?: string; error?: string }> {
    try {
      ...
      if (!createdOrder) {
        const fallbackSnapshot = await db.resolveOrderCityAndFranchiseSnapshot({
          city_id: input.city_id,
          franchise_id: input.franchise_id,
        ...
        createdOrder = {
          ...
          customer_phone: input.customer_phone?.trim() || '',
          customer_id: input.customer_id,
          city_id: fallbackSnapshot.city_id || input.city_id || null,
          franchise_id: fallbackSnapshot.franchise_id || input.franchise_id || null,
      ...
      if (isSupabaseConfigured && supabase) {
        ...
        if (existingOffer) {
          ...
          const rawOfferPayload: Record<string, any> = {
            ...
            expires_at: offerData.expires_at || new Date(Date.now() + 30 * 60 * 1000).toISOString(),
  ...
  public static async rejectOffer(
    ...
    try {
      ...
      if (!assistantIdParam && offerIdOrAssistantId) {
        offerId = orderIdOrOfferId;
