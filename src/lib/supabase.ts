...
export const VALID_TASK_COLUMNS = new Set([
  'id', 'order_id', 'customer_id', 'partner_id', 'assistant_id', 'assistant_name', 'status',
  'task_description', 'pickup_address', 'delivery_address', 'pickup_lat', 'pickup_lng',
  'delivery_lat', 'delivery_lng', 'total_price', 'customer_price', 'courier_net',
  'base_price', 'fuel_cost', 'wear_cost', 'operation_cost', 'tax_cost', 'vat_cost',
...
export const db = {
  ...
  async createOrder(order: Omit<Order, 'id' | 'created_at' | 'status'>): Promise<Order> {
    ...
    if (isSupabaseConfigured && supabase) {
      ...
      let partnerForSnapshot: Partner | null = null;
      if (partnerUuid) {
        try {
          const existingPartner = await this.getPartnerById(partnerUuid);
          if (existingPartner) {
            partnerForSnapshot = existingPartner;
            await ensurePartnerInDatabase(existingPartner);
          } else {
            await ensurePartnerInDatabase({
              id: partnerUuid,
      ...
      // Resolve City and Franchise snapshot safely
      const resolvedSnapshot = await resolveOrderCityAndFranchiseSnapshot({
      ...
      if (cols.has('city_id') && resolvedSnapshot.city_id) {
        payload.city_id = resolvedSnapshot.city_id;
      }
      if (cols.has('franchise_id') && resolvedSnapshot.franchise_id) {
        payload.franchise_id = resolvedSnapshot.franchise_id;
      }
      ...
      const returnedOrder: Order = {
        ...
        city_id: data.city_id || resolvedSnapshot.city_id || order.city_id || null,
        franchise_id: data.franchise_id || resolvedSnapshot.franchise_id || order.franchise_id || null,
      ...
      const newOrder: Order = {
        ...
        city_id: resolvedSnapshot.city_id || order.city_id || null,
        franchise_id: resolvedSnapshot.franchise_id || order.franchise_id || null,
  ...
  async createStoreOrder(params: {
    ...
    const snapshot = await this.resolveOrderCityAndFranchiseSnapshot({
    ...
    if (isSupabaseConfigured && supabase) {
      ...
      const returnedTask = {
        ...
        franchise_id: snapshot.franchise_id || null,
  ...
  async resolveOrderCityAndFranchiseSnapshot(params: {
    city_id?: string | null;
    franchise_id?: string | null;
    partner_id?: string | null;
    city?: string | null;
  ...
  }): Promise<{ city_id: string | null; franchise_id: string | null }> {
    let targetCityId: string | null = params.city_id || null;
    let targetFranchiseId: string | null = params.franchise_id || null;

    // 1. If partner_id is provided, derive snapshot from Partner (Mağazadan Al)
    if (params.partner_id) {
      try {
        const partner = await this.getPartnerById(params.partner_id);
        if (partner) {
          if (partner.city_id) {
            targetCityId = partner.city_id;
          }
          if (partner.franchise_id) {
            targetFranchiseId = partner.franchise_id;
          }
          // If partner has city_id but no franchise_id, attempt resolution
          if (targetCityId && !targetFranchiseId) {
            const res = await this.resolveFranchiseForCity(targetCityId);
            if (res.count === 1 && res.franchiseId) {
              targetFranchiseId = res.franchiseId;
            } else if (res.count > 1 && partner.address) {
              const matched = res.franchises.find(f => 
                Array.isArray(f.districts_covered) && f.districts_covered.some(d => partner.address?.toLowerCase().includes(d.toLowerCase()))
              );
              if (matched) {
                targetFranchiseId = matched.id;
              }
            }
          } else if (!targetCityId && partner.city) {
            const cities = await this.getCities();
            const cleanPartnerCity = partner.city.trim().toLowerCase();
            const matchedCity = cities.find(c => c.name.trim().toLowerCase() === cleanPartnerCity && c.is_active);
            if (matchedCity) {
              targetCityId = matchedCity.id;
              const res = await this.resolveFranchiseForCity(matchedCity.id);
              if (res.count === 1 && res.franchiseId) {
                targetFranchiseId = res.franchiseId;
              }
            }
        ...
        console.warn('Error resolving partner snapshot for order:', e);
      }
    }

    // 2. If not derived from partner, check reliable city information from GPS/structured address
    if (!targetCityId) {
      const rawCity = params.city?.trim() || params.province?.trim() || null;
      if (rawCity) {
        try {
          const cities = await this.getCities();
          const cleanRawCity = rawCity.toLowerCase();
          const matchedCity = cities.find(c => 
            c.name.toLowerCase() === cleanRawCity || 
            cleanRawCity.includes(c.name.toLowerCase()) ||
            c.name.toLowerCase().includes(cleanRawCity)
          );
          if (matchedCity && matchedCity.is_active) {
            targetCityId = matchedCity.id;
            const res = await this.resolveFranchiseForCity(matchedCity.id);
            if (res.count === 1 && res.franchiseId) {
              targetFranchiseId = res.franchiseId;
            } else if (res.count > 1 && params.district) {
              const cleanDistrict = params.district.toLowerCase().trim();
              const matchedFranchise = res.franchises.find(f => 
                Array.isArray(f.districts_covered) && f.districts_covered.some(d => d.toLowerCase().trim() === cleanDistrict)
              );
              if (matchedFranchise) {
                targetFranchiseId = matchedFranchise.id;
              }
            }
          }
        } catch (e) {
          console.warn('Error resolving city from rawCity:', e);
        }
      }
    }

    // 3. Cross-validate franchise belongs to city (Prevent city_id=Sakarya & franchise_id=Kocaeli mismatch)
    if (targetCityId && targetFranchiseId) {
      try {
        const franchise = await this.getFranchise(targetFranchiseId);
        if (franchise && franchise.city_id !== targetCityId) {
          console.warn(`[Integrity Warning] Franchise ${targetFranchiseId} does not belong to City ${targetCityId}. Resetting franchise_id.`);
          targetFranchiseId = null;
        }
      } catch (e) {
        console.warn('Error validating franchise integrity:', e);
      }
    } else if (!targetCityId && targetFranchiseId) {
      try {
        const franchise = await this.getFranchise(targetFranchiseId);
        if (franchise && franchise.city_id) {
          targetCityId = franchise.city_id;
        }
      } catch (e) {}
    }
...
export async function resolveOrderCityAndFranchiseSnapshot(params: {
  city_id?: string | null;
  franchise_id?: string | null;
  partner_id?: string | null;
  city?: string | null;
  ...
  return db.resolveOrderCityAndFranchiseSnapshot(params);
}
